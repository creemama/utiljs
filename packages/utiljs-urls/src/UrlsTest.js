...
describe('Urls', () => {
	...
	describe('#download(url, destination)', () => {
		it('handles HTTP requests', () => download_url_destination('http://www.google.com'))
		it('handles HTTPS requests', () => download_url_destination('https://www.google.com'))
		it('handles HTTP errors', () => download_url_destination('http://doesnotexist').then(
			value => { throw new Error('Unexpected success') },
			error => { /* expected */ }
		))
		it('handles HTTPS errors', () => download_url_destination('https://doesnotexist').then(
			value => { throw new Error('Unexpected success') },
			error => { /* expected */ }
		))
	})
	...
	describe('#download(url, destination, callback)', () => {
		it('handles HTTP requests', callback => download_url_destination_callback('http://www.google.com', callback))
		it('handles HTTPS requests', callback => download_url_destination_callback('https://www.google.com', callback))
		it('handles HTTP errors', callback => download_url_destination_callback('http://doesnotexist',
			error => {
				if (error) return callback()
				callback(new Error('Unexpected success'))
			}
		))
		it('handles HTTPS errors', callback => download_url_destination_callback('https://doesnotexist',
			error => {
				if (error) return callback()
				callback(new Error('Unexpected success'))
			}
	...
	describe('#headers(url)', () => {
		...
		it('handles HTTP errors', () => urls.headers('http://doesnotexist').then(
			value => { throw new Error('Unexpected success') },
			error => { /* expected */ }
		))
		it('handles HTTPS errors', () => urls.headers('https://doesnotexist').then(
			value => { throw new Error('Unexpected success') },
			error => { /* expected */ }
		))
	})
	...
	describe('#headers(url, callback)', () => {
		it('handles HTTP requests', callback => urls.headers('http://www.google.com', callback))
		it('handles HTTPS requests', callback => urls.headers('https://www.google.com', callback))
		it('handles HTTP errors', callback => urls.headers('http://doesnotexist',
			error => {
				if (error) return callback()
				callback(new Error('Unexpected success'))
			}
		))
		it('handles HTTPS errors', callback => urls.headers('https://doesnotexist',
			error => {
				if (error) return callback()
				callback(new Error('Unexpected success'))
			}
