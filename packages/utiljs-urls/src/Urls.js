...
function Urls(options) {
	...
	function files() { return options.files() }
	function http() { return options.http() }
	function https() { return options.https() }
	function promises() { return options.promises() }
	function urlUtil() { return options.url() }
	...
	function download(url, destination, callback) {
		if (!callback) return promises().promisifyAndCall(this, download, url, destination)

		const parsedUrl = urlUtil().parse(url)

		const file = files().createWriteStream(destination)

		getProtocolObject(parsedUrl.protocol).get(url, response => {
			response.pipe(file)
	...
	function headers(url, callback) {
		...
		getProtocolObject(parsedUrl.protocol).request({
				host: parsedUrl.host,
				method: 'HEAD',
				path: parsedUrl.path,
				port: port
