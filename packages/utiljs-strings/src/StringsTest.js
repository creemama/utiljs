...
describe('Strings', () => {
	...
	describe("#base64Url...", () => {
		it('should operate normally', () => {
			...
			expect(strings.base64UrlEncode('ladies and gentlemen we are floating in space'))
				.to.equal('bGFkaWVzIGFuZCBnZW50bGVtZW4gd2UgYXJlIGZsb2F0aW5nIGluIHNwYWNl')
			expect(strings.base64UrlDecode('cmlkZTogZHJlYW1zIGJ1cm4gZG93bg'))
				.to.equal('ride: dreams burn down')
			expect(strings.base64ToBase64Url('qL8R4QIcQ/ZsRqOAbeRfcZhilN/MksRtDaErMA=='))
				.to.equal('qL8R4QIcQ_ZsRqOAbeRfcZhilN_MksRtDaErMA')
			expect(strings.base64UrlToBase64('qL8R4QIcQ_ZsRqOAbeRfcZhilN_MksRtDaErMA'))
			...
			expect(strings.base64UrlToBuffer('c3Bpcml0dWFsaXplZA').toString()).to.equal(buf.toString())
		})
	})
	...
	describe('#endsWith()', () => {
		...
		it('should thow an error when the first or second argument is missing', () => {
			expect(() => { strings.endsWith() }).to.throw(TypeError)
			expect(() => { strings.endsWith('') }).to.throw(TypeError)
		})
	})
	...
	describe('#pad(n, width, z)', () => {
		...
		it('should throw an error when the padding character\'s length is not 1', () => {
			expect(() => { strings.pad('frog', 6, 57) }).to.throw(Error)
			expect(() => { strings.pad(123456, 10, 57) }).to.throw(Error)
		})
	})
	...
	describe('#stripTags(str)', () => {
		it('should operate normally', () => {
			...
			expect(strings.stripTags('<html><>frog</html><span style=\"\"> </span>')).to.equal('<>frog ')
		})
